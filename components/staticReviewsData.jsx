export const staticReviewData = {
    [
        { slug: 'a-way-out-2018', title: 'A Way Out' },
        {
          slug: 'ark-survival-of-the-fittest',
          title: 'ARK: Survival Of The Fittest'
        },
        { slug: 'black-mesa', title: 'Black Mesa' },
        { slug: 'celeste', title: 'Celeste' },
        { slug: 'cuphead', title: 'Cuphead' },
        { slug: 'darkest-dungeon', title: 'Darkest Dungeon' },
        { slug: 'dead-cells', title: 'Dead Cells' },
        { slug: 'disco-elysium', title: 'Disco Elysium' },
        { slug: 'dont-starve-together', title: "Don't Starve Together" },
        { slug: 'fall-guys', title: 'Fall Guys: Ultimate Knockout' },
        { slug: 'firewatch', title: 'Firewatch' },
        { slug: 'hades-2018', title: 'Hades' },
        {
          slug: 'hellblade-senuas-sacrifice',
          title: "Hellblade: Senua's Sacrifice"
        },
        { slug: 'hollow-knight', title: 'Hollow Knight' },
        { slug: 'human-fall-flat', title: 'Human: Fall Flat' },
        { slug: 'inside', title: 'INSIDE' },
        { slug: 'layers-of-fear', title: 'Layers of Fear' },
        { slug: 'no-mans-sky', title: "No Man's Sky" },
        { slug: 'overcooked', title: 'Overcooked' },
        { slug: 'superhot', title: 'SUPERHOT' },
        { slug: 'stardew-valley', title: 'Stardew Valley' },
        { slug: 'subnautica', title: 'Subnautica' },
        { slug: 'the-witness', title: 'The Witness' },
        { slug: 'warhammer-vermintide-2', title: 'Warhammer: Vermintide 2' },
        {
          slug: 'what-remains-of-edith-finch',
          title: 'What Remains of Edith Finch'
        }
      ]
};